...
def test_adversarial_cannot_be_selected_for_generation() -> None:
    """Reserved for PR 2's scenario layer — an error, never an empty benchmark."""
    with pytest.raises(UnavailableDifficultyError) as excinfo:
        selectable_rules(Difficulty.ADVERSARIAL)
    assert "reserved for scenario-based generation" in str(excinfo.value)


def test_the_caller_facing_selection_omits_adversarial_and_adds_mixed() -> None:
    assert {s.value for s in DifficultySelection} == {"bronze", "silver", "gold", "mixed"}
