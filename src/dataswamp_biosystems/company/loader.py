...
# The same canonical configuration, shipped inside the distribution. A source
# checkout always has ``config/`` beside it; a user who merely ``pip install``ed
# the package does not, and without this would have nothing to generate from.
# The packaged copy is a byte-identical copy of the tracked ``config/`` tree
# (see ``[tool.hatch.build.targets.wheel.force-include]``), so which of the two
# is read never changes the generated bytes.
PACKAGED_CONFIG_DIR = Path(__file__).resolve().parent.parent / "_config"

# An *editable* install has neither of the above when run from another working
# directory: it imports from ``src/`` in a checkout that was never built, so
# nothing was force-included. Resolve back to that checkout's tracked ``config/``,
# which is the tree the wheel would have carried.
SOURCE_CONFIG_DIR = Path(__file__).resolve().parents[3] / "config"

# The file whose presence marks a directory as a usable configuration root.
_CONFIG_MARKER = "company.yaml"


def is_config_dir(candidate: Path | str) -> bool:
    """Return whether ``candidate`` looks like a canonical configuration directory."""
    return (Path(candidate) / _CONFIG_MARKER).is_file()


def resolve_config_dir(config_dir: Path | str = DEFAULT_CONFIG_DIR) -> Path:
    """Resolve a configuration directory, falling back to the packaged copy.

    An explicit directory, and the default when a checkout's ``config/`` is
    actually present, are returned unchanged. Only the *default* falls back to
    :data:`PACKAGED_CONFIG_DIR`, and only when the working directory has no
    configuration of its own — an explicit ``--config-dir`` that does not exist
    must still fail loudly, naming the directory the user asked for.
    """
    config_dir = Path(config_dir)
    if config_dir != DEFAULT_CONFIG_DIR or is_config_dir(config_dir):
        return config_dir
    for candidate in (PACKAGED_CONFIG_DIR, SOURCE_CONFIG_DIR):
        if is_config_dir(candidate):
            return candidate
    return config_dir
