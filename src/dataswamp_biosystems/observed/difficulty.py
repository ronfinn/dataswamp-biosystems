...
``adversarial`` is never a rule's default. It is a property of a *scenario*
rather than of a rule: the same rule can appear in a bronze positive case and in
an adversarial near-miss control. It is generated by constructing cases
(:mod:`dataswamp_biosystems.observed.scenarios`) rather than by filtering this
table, which is why :func:`selectable_rules` still refuses it — asking "which
rules are adversarial?" is the wrong question. Relabelling an ordinary rule
adversarial merely to populate the tier would be a lie about what the tier means.
"""
...
class DifficultySelection(StrEnum):
    ...
    *rule*; this enum is the generation-time choice and adds ``mixed`` — the full
    rule catalogue, the pre-existing default.

    ``mixed`` means exactly the ordinary rule tiers (bronze, silver and gold
    together) and never includes adversarial scenarios. That is a deliberate
    boundary, not an oversight: adversarial cases are constructed rather than
    sampled, so folding them into ``mixed`` would change the default benchmark's
    canonical bytes and every published baseline score with them. A future
    all-tier mode, if one is ever wanted, must be a new explicit selection.

    ``adversarial`` is selectable but is *not* a rule filter — it switches
    generation to the scenario engine (:mod:`dataswamp_biosystems.observed.scenarios`),
    which is why :func:`selectable_rules` still refuses it.
    """
...
class UnavailableDifficultyError(ValueError):
    """A tier was requested as a *rule filter* that no rule can populate."""

    def __init__(self, difficulty: Difficulty) -> None:
        self.difficulty = difficulty
        super().__init__(
            f"difficulty {difficulty.value!r} is produced by the scenario engine, "
            f"not by filtering the rule registry; it cannot be resolved to a rule "
            f"set. Rule-filtered tiers are "
...
def selectable_rules(difficulty: Difficulty) -> tuple[str, ...]:
    ...
    today only ``adversarial``, which is generated by constructing scenarios
    (:mod:`dataswamp_biosystems.observed.scenarios`) rather than by filtering the
    ordinary registry. Returning an empty tuple instead would silently produce a
    defect-free "adversarial" benchmark, which is worse than an error because it
    looks like a result.
    """
